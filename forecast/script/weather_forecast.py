...
import calendar
from selinium_weather import sel
...
def forecast(file_name,yesterday):
    ...
    y = (yesterday.year, yesterday.month, yesterday.day)
    if len(a)!=len(b):
        date_index=date.index(y)
        max_temp.insert(date_index,max_temp[date_index-1])
        min_temp.insert(date_index,min_temp[date_index-1])
...
if __name__ == '__main__':
    ...
    sel(url)
    forecast_table = forecast(get_resources_path() + '/forecast.html',yesterday)
